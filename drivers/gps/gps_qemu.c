...
//*****************************************************************************
	state->fd= open("/dev/ttymxc2",O_RDWR|O_NOCTTY|O_NDELAY);//这里用的是UART1

     if( state->fd < 0){

            LOGE("open port /dev/ttymxc2 ERROR..state->fd=%d\n",state->fd); 

            exit(0);

        }else

            LOGE("open port:/dev/ttymxc2 succceed..state->fd=%d\n",state->fd);
