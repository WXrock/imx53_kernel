...
static void flexcan_set_bitrate(struct flexcan_device *flexcan, int bitrate)
{
	...
	if (found) {
		...
		flexcan->bitrate = bitrate;
                //flexcan->br_presdiv = 7;
		//flexcan->br_propseg = 2;
		//flexcan->br_pseg1 = 3;
		//flexcan->br_pseg2 = 3;
		//flexcan->bitrate = bitrate;
                //printk(KERN_ERR "flexcan->br_presdiv=%d!\n",flexcan->br_presdiv);
	} else {
				...
				size_t count)
{
	...
	struct net_device *net;
	struct flexcan_device *flexcan;
        printk(KERN_ERR "flexcan_set_attr\n");
	net = dev_get_drvdata(dev);
	BUG_ON(!net);
	flexcan = netdev_priv(net);
	BUG_ON(!flexcan);
	...
	tmp = simple_strtoul(buf, NULL, 0);
        printk(KERN_ERR "tmp=%d\n",tmp);
        printk(KERN_ERR "attr_id:%d\n",attr_id);
	switch (attr_id) {
	case FLEXCAN_ATTR_BITRATE:
					...
					void (*setup) (struct net_device *dev))
{
	...
	struct net_device *net;
	int i, num;
        //printk(KERN_ERR "Allocate netdevice for FlexCAN !\n");
