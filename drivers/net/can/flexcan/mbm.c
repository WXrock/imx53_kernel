...
static void flexcan_mb_bottom(struct net_device *dev, int index)
{
	...
	struct sk_buff *skb;
	unsigned int tmp;
        //printk(KERN_ERR "flexcan_mb_bottom\n");
	...
	if (skb) {
		...
		if (frame->can_dlc && frame->can_dlc)
			flexcan_memcpy(frame->data, hwmb->mb_data,
				       frame->can_dlc);
                //printk(KERN_ERR "frame->can_id=%x\n",frame->can_id);
                //printk(KERN_ERR "frame->data[0]=%d\n",frame->data[0]);
                //printk(KERN_ERR "frame->data[1]=%d\n",frame->data[1]);
                //printk(KERN_ERR "frame->data[2]=%d\n",frame->data[2]);
                //printk(KERN_ERR "frame->data[3]=%d\n",frame->data[3]);
                if (flexcan->fifo
...
int flexcan_mbm_xmit(struct flexcan_device *flexcan, struct can_frame *frame)
{
	...
	struct can_hw_mb *hwmb = flexcan->hwmb;
       //printk(KERN_ERR "flexcan_xmit_mb=%d",flexcan->xmit_mb);
	do {
		if ((hwmb[i].mb_cs & MB_CS_CODE_MASK) >> MB_CS_CODE_OFFSET ==
							    CAN_MB_TX_INACTIVE)
			break;
                //printk(KERN_ERR "flexcan_mbm_xmit\n");
		if ((++i) > flexcan->maxmb) {
			if (flexcan->fifo)
	...
	if (flexcan->xmit_mb > flexcan->maxmb) {
        ...
        //printk(KERN_ERR "xmit_mb=%d",flexcan->xmit_mb);
	if (frame->can_id & CAN_RTR_FLAG)
        {
               
                //printk(KERN_ERR "CAN_RTR_FLAG");
	...
	if (frame->can_id & CAN_EFF_FLAG) {
        ...
        //printk(KERN_ERR "Length=%d",frame->can_dlc);
        //printk(KERN_ERR "data0=%d,data1=%d,data2=%d,data3=%d,data4=%d,data5=%d,data6=%d,data7=%d\n",hwmb[i].mb_data[0],hwmb[i].mb_data[1],hwmb[i].mb_data[2],
//hwmb[i].mb_data[3],hwmb[i].mb_data[4],hwmb[i].mb_data[5],hwmb[i].mb_data[6],
//hwmb[i].mb_data[7]);
        //reg = __raw_readl(flexcan->io_base + CAN_HW_REG_IMASK2);
        //printk(KERN_ERR "CAN_HW_REG_IMASK2:%x\n",reg); 
        //reg = __raw_readl(flexcan->io_base + CAN_HW_REG_IMASK1);
        //printk(KERN_ERR "CAN_HW_REG_IMASK1:%x\n",reg); 
        //reg = __raw_readl(flexcan->io_base + CAN_HW_REG_IFLAG2);
        //printk(KERN_ERR "CAN_HW_REG_IFLAG2:%x\n",reg); 
        //reg = __raw_readl(flexcan->io_base + CAN_HW_REG_IFLAG1);
        //printk(KERN_ERR "CAN_HW_REG_IFLAG1:%x\n",reg); 
        //reg = __raw_readl(flexcan->io_base + CAN_HW_REG_MCR);
        //printk(KERN_ERR "CAN_HW_REG_MCR:%x\n",reg); 
        //reg = __raw_readl(flexcan->io_base + CAN_HW_REG_CTRL);
        //printk(KERN_ERR "CAN_HW_REG_CTRL:%x\n",reg); 
        //reg = __raw_readl(flexcan->io_base + CAN_HW_REG_ESR);
        //printk(KERN_ERR "CAN_HW_REG_ESR:%x\n",reg);
        //reg = __raw_readl(flexcan->io_base + CAN_HW_REG_TIMER);
        //printk(KERN_ERR "CAN_HW_REG_TIMER=%x\n",reg);
	...
	hwmb[i].mb_cs |= CAN_MB_TX_ONCE << MB_CS_CODE_OFFSET;
        //printk(KERN_ERR "CAN_MB_STATUS[33]=%x\n",hwmb[33].mb_cs);
        //printk(KERN_ERR "CAN_MB_STATUS[34]=%x\n",hwmb[34].mb_cs);
        //printk(KERN_ERR "CAN_MB_STATUS[35]=%x\n",hwmb[35].mb_cs);
	return 0;
}
