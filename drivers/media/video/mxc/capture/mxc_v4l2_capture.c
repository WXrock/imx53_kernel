...
#define Xrock_DEBUG 1

#if Xrock_DEBUG
#define wx_dbg(fmt, arg...) printk("\t<Xrock> mxc_v4l2_capture.c %s:"fmt"\n",__func__,##arg)
#else
#define wx_dbg(arg...) do {} while(0)
#endif
...
static int mxc_allocate_frame_buf(cam_data *cam, int count)
{
	...
	wx_dbg("In MVC:mxc_allocate_frame_buf - size=%d,count=%d",
		cam->v2f.fmt.pix.sizeimage,count);
...
static int mxc_v4l2_s_fmt(cam_data *cam, struct v4l2_format *f)
{
	...
	wx_dbg("f->fmt.pix.width:%d\nf->fmt.pix.height:%d\nf->fmt.pix.colorspace:%d\n",
        f->fmt.pix.width,f->fmt.pix.height,f->fmt.pix.colorspace);
			    ...
			    unsigned int ioctlnr, void *arg)
{
	...
	case VIDIOC_S_INPUT: {
		int *index = arg;
		wx_dbg("   case VIDIOC_S_INPUT:%d",*index);
	...
	case VIDIOC_ENUM_FRAMESIZES: {
        ...
        wx_dbg("   case VIDIOC_ENUM_FRAMESIZES");
		if (cam->sensor)
	...
	case VIDIOC_DBG_G_CHIP_IDENT: {
        ...
        wx_dbg("    case VIDIOC_DBG_G_CHIP_IDENT");
