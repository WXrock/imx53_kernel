...
static void ldb_enable(int ipu_di)
{
	...
	case LDB_DUL_DI0:
	case LDB_SPL_DI0:
		tx_dbg("LDB_DUL_DI0 or LDB_SPL_DI0");
		if (ipu_di != 0)
			return;
		else
			goto proc;
	case LDB_DUL_DI1:
	case LDB_SPL_DI1:
		tx_dbg("LDB_DUL_DI1 or LDB_SPL_DI1");
		if (ipu_di != 1)
			return;
proc:
...
static int ldb_probe(struct platform_device *pdev)
{
	...
	tx_dbg("spin_lock_init");
	spin_lock_init(&ldb_lock);

	g_ldb_dev = &pdev->dev;

	tx_dbg("platform_get_resource");
