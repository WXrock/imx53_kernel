...
static struct fb_info *mxcfb_init_fbinfo(struct device *dev, struct fb_ops *ops)
{
	 ...
	 */
	tx_dbg("Allocate sufficient memory for the fb structure,-> framebuffer_alloc.");
...
{
	tx_dbg(" pdev->name:%s ,pdev->id:%d  ",pdev->name,pdev->id);
	struct fb_info *fbi;
	struct mxcfb_info *mxcfbi;
	struct resource *res;
	char *options;
	 ...
	 */
	tx_dbg("Ininialize FB structures,-> mxcfb_init_fbinfo.");
