...
#if TXMCU_DEBUG
#define tx_dbg(fmt, arg...) printk("\t<TX-MCU> devices.c %s:"fmt"\n",__func__,##arg)
#else
#define tx_dbg(arg...)
#endif
...
int __init mxc_register_gpios(void)
{
	...
	tx_dbg("return mxc_gpio_init(mxc_gpio_ports, ARRAY_SIZE(mxc_gpio_ports)).");
...
{
	tx_dbg("MACHINE_START - .init_irq = mx5_init_irq, => here.");
	...
	tx_dbg("-> mxc_tzic_init_irq(%ld)",tzic_addr);
