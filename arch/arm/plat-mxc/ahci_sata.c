...
#if TXMCU_DEBUG
#define tx_dbg(fmt, arg...) printk(" **TX-MCU** %s %s:"fmt"\n",__FILE__,__func__,##arg)
#else
#define tx_dbg(arg...)
#endif
...
static int sata_init(struct device *dev)
{
	...
	tx_dbg("Get the AHB clock rate, and configure the TIMER1MS reg later.");
	...
	if (machine_is_mx53_smd() || machine_is_mx53_loco()
			|| board_is_mx53_ard_b()) {
		tx_dbg("FSL IMX AHCI SATA uses the internal usb phy1 clk.");
		...
		tx_dbg("External CLK input is used for AHCI.");
	...
	if (AHCI_SAVE_PWR_WITHOUT_HOTPLUG) {
		tx_dbg("Release resources when there is no de ice on the port.");
	...
	tx_dbg("Add the temperature monitor.");
